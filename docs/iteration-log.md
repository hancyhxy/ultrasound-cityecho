...
### ITER-7 · 2026-05-06 · First true visual design — pivot from "AI-generated default" to intended visual system

| Field | Content |
|---|---|
| **Pain point** | Critical reframing surfaced by user: the v1 visual system (midnight-indigo + amber accent + Fraunces serif + glass cards + JetBrains-Mono captions) was **never designed** — it was the default output of Lovable.dev's prototype generator, which I then continued in the same idiom across ITER-1 through ITER-6 while doing information architecture work. The product has been wearing AI-generated clothes the whole time. ITER-7 is therefore not a "skin swap" or "token migration" of an existing design — it is the **first iteration where this product has visual intent at all**. |
| **Source of insight** | User direct statement (2026-05-06): *"之前那个你可以理解为是没有进行过干预的，只是一个 AI 生成的视觉。而这一版的视觉才是我们要去做的。"* This reframing eliminated the "preserve heritage" constraint I had been honoring throughout ITER-5/6 — there is no heritage to preserve. |
| **Severity** | **3 — Major friction.** A product without visual intent reads as a tech demo regardless of how good its information architecture is. Until ITER-7 ships, the Experience Evaluation deliverable's visual axis (Voice & tone, Emotional resonance) cannot honestly claim a designed system. |
| **Dimension(s) affected** | Voice & tone (the dominant axis for this iteration) · Emotional resonance · Companionship in solitude (warmer, more "friend's room" reading after the pivot) · Pace & breathing room (tighter chunky pop layout vs. art-house whitespace) |
| **Hypothesis** | A clean visual pivot is achievable in one iteration because (a) all v1 token usage is centralised through CSS variables in `styles.css` (~200 references across 15 files but they all resolve to ~10 token names), (b) features and IA from ITER-1/6 are stable, so there's no in-flight work to disturb, (c) `design.md` (ITER-5) already codified the target system rule by rule. The pivot consists of three coordinated moves: token rewrite + Fraunces full removal + `--warm` rename to `--accent`. |
| **User-confirmed boundaries** | (1) Fraunces serif is **fully removed**, not demoted as `design.md` originally proposed — `/me` chapter numerals, trace note italics, footer line all become Inter italic. The literary residue is reframed through *italic + quotes + the writing itself* rather than through a serif typeface. (2) `--warm` is renamed to `--accent` codebase-wide so the variable name no longer says "amber" while the hue is now magenta. (3) `/playing` 6-tile dashboard structure stays as-is (visual tokens shift but layout doesn't) — its single-portrait redesign is deferred to ITER-8 per `design.md` §6 exception. (4) `/playlist` UX is left as Lovable-original (deferred to ITER-9+). |
| **Change made** | (1) `src/styles.css` rewritten: `:root` tokens migrate from midnight-indigo + amber to plum-black + neon magenta-purple gradient pair + hot-pink highlight; new tokens added (`--primary-bright`, `--accent-hot`, `--gradient-night`, `--gradient-neon`, `--shadow-neon`, `--radius-pill`, `--radius-card-lg`, `--radius-card-xl`, `--radius-portrait`); `--font-display` and `--font-mono` aliased to Inter so legacy `font-display` / `font-mono` callsites don't break. New utility classes: `.text-gradient-neon`, `.shadow-neon`, `.bg-gradient-night`, `.bg-gradient-neon`, `.rounded-portrait`, `.rounded-card-lg`, `.rounded-card-xl`. (2) Codebase-wide rename: `bg-warm` → `bg-accent`, `text-warm` → `text-accent`, `shadow-warm` → `shadow-accent`, `border-warm` → `border-accent`, `from-warm`/`via-warm`/`to-warm` → `accent` equivalents, plus opacity variants (`bg-warm/15`, `text-warm/80` etc.). (3) Per-page polish: `/index` and `/traces` headlines become Inter 800 uppercase tracking-tight; `/me` chapter numerals become Inter 800 italic, footer becomes Inter 700 italic; user portrait gradient on `/me` becomes `from-accent via-primary to-primary-bright` + `shadow-neon`; `PhoneShell` bottom-nav active tab becomes a filled pill with neon gradient; `traces` story-bubble unread ring becomes neon (`from-accent via-primary-bright to-primary`); `router.tsx` `DefaultErrorComponent` switches red destructive to hot-pink accent. (4) `font-mono` callsites in product code (chapter labels, captions, time stamps, mood pill text) replaced with `font-semibold tracking-wider` Inter — but `font-mono` token retained as Inter alias for stragglers. |
| **Outcome** | Shipped 2026-05-06. HMR walkthrough on `localhost:8080`: every screen now reads as a designed product, not an AI default. Background is plum-black with three drifting orbs in neon purple/magenta; story-bubble unread rings glow neon; user portrait on `/me` is a 24×24 rounded-portrait gradient (accent-hot → primary → primary-bright) sitting inside `shadow-neon`; chapter numerals (I. II. III. IV.) on `/me` are Inter 800 italic with `text-gradient-neon`; footer hero "Somewhere in the city" reads as bold gradient over a quieter follow-up clause. Bottom nav now expands the active tab into a filled gradient pill while inactive tabs collapse to icon-only — Instagram/airbuds idiom directly. Route titles on `/`, `/traces` are Inter 800 uppercase with hot-pink kicker captions. Error route reskinned with hot-pink portrait icon and pill CTAs. `--warm` rename to `--accent` complete (with `--color-warm` legacy alias preserved at the @theme level so any straggler resolves). |
| **Trade-offs** | (1) **Loss of literary signature.** Removing Fraunces makes the product look more "modern social app" and less "art-house companion" — closer to airbuds.fm visual idiom. The product voice (one-sentence traces, "a stranger" anonymity, place-bound notes) compensates. If walkthrough reveals the loss of "gravitas" is too sharp, the rollback is small: re-import Fraunces and apply only to `<blockquote>`-shaped trace notes. (2) **`/playing` visual incoherence with rest of app post-ITER-7.** Dashboard 6-tile layout in plum + neon will read as awkward until ITER-8 redesigns it as single-portrait. Acceptable for one iteration. Logged as ITER-8 priority. (3) **`--warm` rename costs ~80 sed touches.** Worth it for code clarity (no future reader will be confused that `--warm` is purple). (4) **Bottom-nav filled-pill might cramp 4 tabs in 420px PhoneShell width.** If observed in walkthrough, fall back to icon-only with pill behind icon (label hidden when active). |
| **Commit / PR** | TBD |
